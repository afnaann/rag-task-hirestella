...
As requested by the assessment brief, here is a transparent accounting of what was intentionally omitted:

1. **Token Streaming**: The API returns structured batch JSON (`{ answer, sources, grounded, refused, provider }`). Token streaming via `ReadableStream` was omitted to keep citation attribution, latency telemetry, and automated evaluation parsing clean and deterministic within the 8–10 hour time budget.
2. **Multi-Turn Conversation Memory**: Each query is treated as an independent single-turn question. Conversational session memory and follow-up pronoun resolution (e.g., *"What else did he do there?"*) were omitted to evaluate raw retrieval and grounding accuracy without conversational drift.
3. **Token-Precise Budgeting**: Chunks are segmented using character lengths (~4 chars/token approximation) rather than a native BPE tokenizer (e.g. `tiktoken`).
4. **Incremental Document Ingestion**: Ingestion (`npm run ingest`) performs a full clean rebuild of `data/vector-store.json`. While fast for ~50 chunks (<10 seconds), an enterprise system would require incremental document hashing and change-data-capture.
